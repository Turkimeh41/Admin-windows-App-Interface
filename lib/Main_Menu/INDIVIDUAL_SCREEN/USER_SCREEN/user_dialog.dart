...
class UserDialog {
  ...
  static void searchDialog(BuildContext context, Users insUsers, AnimationController animationController) {
    showDialog(
        context: context,
        builder: (context) {
          ...
          Animation<double> animation = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: animationController, curve: Curves.linear));
                                    ...
                                    child: GestureDetector(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          const Icon(Icons.arrow_drop_down, color: Color.fromARGB(255, 231, 215, 215))
                                        ],
                                      ),
                                    ),
                                  ),
                                          ...
                                          SizedBox(
                                            ...
                                            child: ListView.separated(
                                                itemBuilder: (context, i) {
                                                  final singleUser = insUsers.filter(searchController.text, filter, 0)[i];
                                                  return UserWidget(
                                                    user: singleUser,
                                                ...
                                                itemCount: insUsers.filter(searchController.text, filter, 0).length),
                                          ),
                                                ...
                                                return id == ''
                                                    ...
                                                    : Positioned(
                                                        ...
                                                        child: FadeTransition(
                                                          opacity: animation,
                                                          child: EditContainer(userID: id, setState: setStateful),
                                                        ),
                                                      );
                                              })
                                        ],
  ...
  static void deleteDialog({required BuildContext context, required Users insUsers, required String id}) {
    showDialog(
        context: context,
        builder: (context) {
          bool loading = false;
  ...
  static Future<void> switchDialog({required BuildContext context, required Users insUsers, required String userID, required bool status}) async {
    await showDialog(
        context: context,
        builder: (context) {
          bool loading = false;
